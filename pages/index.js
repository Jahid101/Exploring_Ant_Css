      ...
      <Layout className="layout">
        ...
        <Header>
          ...
          <Menu theme="dark" mode="horizontal" defaultSelectedKeys={['0']}>
            {new Array(15).fill(null).map((_, index) => {
              const key = index + 1;
              const sense = ["", "treeSelect", "upload", "avatar", "badge", "calender", "card", "carousel", "collapse", "comment", "description", "empty", "image", "list", "popover", "statistic"]
              return <Menu.Item
                onClick={() => handleClick(sense[key])}
                key={key}
              >
        ...
        <br />
        <h1 className="text-center">Session: 04</h1>
        <Header>
          <div className="logo" />
          <Menu theme="dark" mode="horizontal" defaultSelectedKeys={['0']}>
            {new Array(15).fill(null).map((_, index) => {
              const key = index + 1;
              const sense = ["", "table", "tabs", "timeline", "tooltip", "tree", "alert", "drawer", "message", "modal", "notification", "popconfirm", "progress", "result", "skeleton", "spin"]
              return <Menu.Item
                onClick={() => handleClick(sense[key])}
                key={key}
              >
